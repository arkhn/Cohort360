...
const PyramidChart: React.FC<PyramidProps> = memo(
  ({ data, width = 400, height = 250 }) => {
    ...
    useEffect(() => {
      ...
      let valueMax = 0
      let customData: { age: number; male: number; female: number }[] = []
      ...
      const y = d3
        .scaleLinear()
        .domain([
          d3.min(customData, (d) => d.age),
          d3.max(customData, (d) => d.age)
        ])
        .nice()
      ...
      svg
        .append('path')
        .attr('d', femaleAreaGenerator(customData))
        .style('fill', '#FC568F')

      svg
        .append('path')
        .attr('d', maleAreaGenerator(customData))
        .style('fill', '#78D4FA')
